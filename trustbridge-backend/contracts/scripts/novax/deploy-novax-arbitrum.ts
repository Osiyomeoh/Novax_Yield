...
async function main() {
  ...
  console.log("Deploying Novax contracts to Arbitrum One with account:", deployer.address);
  console.log("Account balance:", ethers.formatEther(await ethers.provider.getBalance(deployer.address)), "ETH");

  // Arbitrum Chainlink addresses (mainnet)
  // Note: Update these with actual Arbitrum Chainlink addresses
  const ARBITRUM_CHAINLINK = {
    // Price Feeds (update with actual Arbitrum addresses)
    ETH_USD: process.env.ARBITRUM_ETH_USD_FEED || ethers.ZeroAddress,
    BTC_USD: process.env.ARBITRUM_BTC_USD_FEED || ethers.ZeroAddress,
    USDC_USD: process.env.ARBITRUM_USDC_USD_FEED || ethers.ZeroAddress,
    LINK_USD: process.env.ARBITRUM_LINK_USD_FEED || ethers.ZeroAddress,
    // VRF (update with actual Arbitrum addresses)
    VRF_COORDINATOR: process.env.ARBITRUM_VRF_COORDINATOR || ethers.ZeroAddress,
    VRF_KEY_HASH: process.env.ARBITRUM_VRF_KEY_HASH || ethers.ZeroHash,
    VRF_SUBSCRIPTION_ID: process.env.ARBITRUM_VRF_SUBSCRIPTION_ID || "0",
    // Functions (update with actual Arbitrum addresses)
    FUNCTIONS_ORACLE: process.env.ARBITRUM_FUNCTIONS_ORACLE || ethers.ZeroAddress,
    FUNCTIONS_SOURCE_HASH: process.env.ARBITRUM_FUNCTIONS_SOURCE_HASH || ethers.ZeroHash,
    FUNCTIONS_SUBSCRIPTION_ID: process.env.ARBITRUM_FUNCTIONS_SUBSCRIPTION_ID || "0",
    FUNCTIONS_GAS_LIMIT: process.env.ARBITRUM_FUNCTIONS_GAS_LIMIT || "100000",
  };

  // USDC address on Arbitrum One (native USDC)
  ...
  const filePath = path.join(deploymentsDir, `novax-arbitrum-${chainId}.json`);
